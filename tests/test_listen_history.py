    ...
    def _insert_test_data(self, data):
        ...
        for song_uid, listened_at in data:
            cursor.execute(
                ...
                (song_uid, listened_at.isoformat()),
            )
        conn.commit()
        conn.close()

    def _insert_song_metadata(self, songs):
        """Helper to insert song metadata

        Args:
            songs: List of tuples (uid, title)
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute(
        ...
        for uid, title in songs:
            cursor.execute(
                "INSERT OR IGNORE INTO songs (uid, title) VALUES (?, ?)", (uid, title)
            )
        conn.commit()
        conn.close()
...
class TestDatabaseInitialization(TestListenHistory):
    ...
    def test_init_database_creates_directory(self):
        ...
        nested_path = os.path.join(
            tempfile.gettempdir(), "test_nested", "subdir", "test.db"
        )
...
class TestLogListen(TestListenHistory):
    ...
    def test_log_listen_valid_uid(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT song_uid FROM listen_history WHERE song_uid = ?", (song_uid,)
        )
        result = cursor.fetchone()
        conn.close()
    ...
    def test_log_listen_timestamp(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT listened_at FROM listen_history WHERE song_uid = ?", (song_uid,)
        )
        result = cursor.fetchone()
        conn.close()
...
class TestTopSongsQueries(TestListenHistory):
    ...
    def test_get_top_songs_all_time_basic(self):
        ...
        self._insert_song_metadata(
            [
                ("song1234ABCD5678", "Test Song 1"),
                ("song5678EFGH1234", "Test Song 2"),
                ("songABCDEFGH0000", "Test Song 3"),
            ]
        )
        ...
        self.assertEqual(
            result[0],
            {"uid": "song1234ABCD5678", "title": "Test Song 1", "listen_count": 3},
        )
        self.assertEqual(
            result[1],
            {"uid": "song5678EFGH1234", "title": "Test Song 2", "listen_count": 2},
        )
        self.assertEqual(
            result[2],
    ...
    def test_get_top_songs_all_time_with_limit(self):
        ...
        self.assertEqual(result[0]["uid"], "song3333CCCC3333")
        self.assertEqual(result[1]["uid"], "song2222BBBB2222")
    ...
    def test_get_top_songs_all_time_reverse(self):
        ...
        self.assertEqual(
            result[0], {"uid": "song1111AAAA1111", "title": "Song A", "listen_count": 1}
        )
        self.assertEqual(
            result[1], {"uid": "song2222BBBB2222", "title": "Song B", "listen_count": 2}
        )
        self.assertEqual(
            result[2], {"uid": "song3333CCCC3333", "title": "Song C", "listen_count": 3}
        )
    ...
    def test_get_top_songs_last_n_days(self):
        ...
        self._insert_song_metadata(
            [
                ("songRECENT000001", "Recent Song"),
                ("songOLD0000000001", "Old Song"),
            ]
        )
        ...
        self.assertEqual(
            result[0],
            {"uid": "songRECENT000001", "title": "Recent Song", "listen_count": 2},
        )
    ...
    def test_get_top_songs_this_week(self, mock_datetime):
        ...
        self._insert_song_metadata(
            [
                ("songWEEK00000001", "This Week Song"),
                ("songOLDWEEK00001", "Last Week Song"),
            ]
        )
        ...
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]["uid"], "songWEEK00000001")
    ...
    def test_get_top_songs_this_month(self, mock_datetime):
        ...
        self._insert_song_metadata(
            [
                ("songMONTH0000001", "This Month Song"),
                ("songOLDMONTH0001", "Last Month Song"),
            ]
        )
        ...
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]["uid"], "songMONTH0000001")
    ...
    def test_get_top_songs_this_year(self, mock_datetime):
        ...
        self._insert_song_metadata(
            [
                ("songYEAR00000001", "This Year Song"),
                ("songOLDYEAR00001", "Last Year Song"),
            ]
        )
        ...
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]["uid"], "songYEAR00000001")
...
class TestSpecificPeriodQueries(TestListenHistory):
    ...
    def test_get_top_songs_for_week(self):
        ...
        # Using dates in January 2025: Jan 6-12 is week 1 (Mon-Sun)
        # and Jan 13-19 is week 2
        test_data = [
            ("songWEEK01_0001", datetime(2025, 1, 6)),  # Week 1 - Monday
            ("songWEEK01_0001", datetime(2025, 1, 10)),  # Week 1 - Friday
            ("songWEEK02_0001", datetime(2025, 1, 13)),  # Week 2 - Monday
        ]
        self._insert_test_data(test_data)
        self._insert_song_metadata(
            [
                ("songWEEK01_0001", "Week 1 Song"),
                ("songWEEK02_0001", "Week 2 Song"),
            ]
        )

        result = listen_history.get_top_songs_for_week(2025, 1, db_path=self.db_path)

        self.assertEqual(len(result), 1)
        self.assertEqual(
            result[0],
            {"uid": "songWEEK01_0001", "title": "Week 1 Song", "listen_count": 2},
        )
    ...
    def test_get_top_songs_for_month(self):
        ...
        self._insert_song_metadata(
            [
                ("songDEC00000001", "December Song"),
                ("songNOV00000001", "November Song"),
            ]
        )
        ...
        self.assertEqual(
            result[0],
            {"uid": "songDEC00000001", "title": "December Song", "listen_count": 2},
        )
    ...
    def test_get_top_songs_for_year(self):
        ...
        self._insert_song_metadata(
            [
                ("song2025_000001", "2025 Song"),
                ("song2024_000001", "2024 Song"),
            ]
        )
        ...
        self.assertEqual(
            result[0],
            {"uid": "song2025_000001", "title": "2025 Song", "listen_count": 2},
        )
...
class TestEdgeCases(TestListenHistory):
    ...
    def test_single_song_multiple_listens(self):
        ...
        self.assertEqual(
            result[0],
            {"uid": "songSINGLE00001", "title": "Single Song", "listen_count": 3},
        )
    ...
    def test_limit_larger_than_results(self):
        ...
        self._insert_song_metadata(
            [
                ("songLIMIT0000001", "Limit Test Song"),
            ]
        )
        ...
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]["uid"], "songLIMIT0000001")
