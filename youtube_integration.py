...
def download_and_add_video(url: str) -> Optional[str]:
    ...
    if action == "skip_url":
        ...
        print(f"📝 URL tracked but missing data, downloading to update...")
        existing_song = youtube_utils.get_song_by_url(url)
...
def download_and_add_playlist(url: str) -> Optional[str]:
    ...
    if not playlist_info or not playlist_info["downloaded_songs"]:
        print(f"✗ Playlist download failed or no songs downloaded")
        return None
    ...
    try:
        ...
        playlist_uid = playlists.create_playlist(
            name=playlist_name,
            description=f"Downloaded from YouTube: {url}"
        )
        ...
        if song_uids:
            playlists.add_songs_to_playlist(playlist_uid, song_uids)
...
def add_existing_url_to_playlist(url: str, playlist_uid: str) -> bool:
    ...
    # Add to playlist
    try:
        playlists.add_song_to_playlist(playlist_uid, song_uid)
...
def redownload_song(uid: str) -> bool:
    ...
    print(f"Duration updated: {existing_song.get('duration', 0)}s → {download_info['duration']}s")
